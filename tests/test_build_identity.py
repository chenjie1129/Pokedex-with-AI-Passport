...
class IdentityTests(unittest.TestCase):
    def test_incremental_build_refreshes_version(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            (repo/'VERSION').write_text('1.2.3-dev\n')
            ...
            self.assertEqual(original['version'], '1.2.3-dev')
            self.assertEqual(original['build_id'], original['git_commit'][:12])
            header=(repo/'build/identity/city_build_identity.h').read_text()
            self.assertIn('#define CITY_BUILD_ID ', header)
            self.assertNotIn('CITY_BUILD_BUILD_ID', header)
            with (repo/'main/main.c').open('a') as f: f.write('// edited\n')
            run('cmake','--build','build')
            edited=built()
            self.assertTrue(edited['dirty'])
            self.assertEqual(original['version'], edited['version'])
            self.assertNotEqual(original['build_id'], edited['build_id'])
            self.assertTrue(edited['build_id'].endswith('-dirty'))
            (repo/'main/new.h').write_text('// untracked\n')
            run('cmake','--build','build')
            self.assertEqual(edited['version'], built()['version'])
            self.assertEqual(edited['build_id'], built()['build_id'])
            self.assertIn('main/new.h',built()['source_files'])
            run('git','add','.');run('git','commit','-m','updated')
            run('cmake','--build','build')
            self.assertFalse(built()['dirty'])
            self.assertEqual(built()['build_id'], built()['git_commit'][:12])
    ...
    def test_snapshot_and_package(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            (repo/'.gitignore').write_text('build/\nsdkconfig\n')
            (repo/'VERSION').write_text('0.1.0-dev\n')
            ...
            self.assertEqual(clean['version'], '0.1.0-dev')
            self.assertEqual(clean['build_id'], clean['git_commit'][:12])
            (repo/'main.c').write_text('v2')
            dirty=identity(repo)
            self.assertTrue(dirty['dirty'])
            self.assertEqual(dirty['version'], clean['version'])
            self.assertTrue(dirty['build_id'].endswith('-dirty'))
            self.assertNotEqual(clean['source_sha256'],dirty['source_sha256'])
            (repo/'main.c').write_text('v3')
            self.assertNotEqual(dirty['source_sha256'],identity(repo)['source_sha256'])
