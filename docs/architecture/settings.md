...
- The bottom of Settings shows the semantic firmware version from the root
  `VERSION` file. The short build ID, full commit, source fingerprint and
  dirty state remain in boot logs and release manifests for support and
  evidence without making the user-facing version difficult to read.
...
save/read errors, saving, low battery and the maximum-length semantic version
marker.
